...
$app->post('/restProfile/public', function($request, $response, $args){
    ...
    if(!empty($data)) {
        ...
        //if session exists and not expires and not restaurant account
            ...
            if (empty($vote)) {
                $this->response->getBody()->write(addDish($this->db, $dish_id, $user_id, $id));
		return $this->response->withAddedHeader('Access-Control-Allow-Origin', '*');
            ///////////if vote is not empty -> changing vote
            } else {
                $this->response->getBody()->write(updateVote($this->db, $dish_id, $user_id, $id, $vote));
		return $this->response->withAddedHeader('Access-Control-Allow-Origin', '*');
            }
        }
...
$app->get('/restaurant/[{session_id}]', function($request, $response, $args)
{
        if(checkSession($this, $args['session_id']) === 'r')
        {
                ...
                $restQuery = $this->db->prepare("SELECT name, city, state_post_code, category, rest_id, address" .
...
$app->post('/restaurant/deleteDishes', function($request, $response)
{
        // Query sessions table against recieved session ID, store in $currentSession as row object
        $rawData = $this->request->getBody();
        $sessionData = json_decode($rawData, True);
...
$app->post('/restaurant/updateInfo', function($request, $response)
{
	$sessionData = json_decode($request->getBody(), TRUE);
	if(checkSession($this, $sessionData['session_id']) === 'r')
	{
		...
		$updateQuery = $this->db->prepare("UPDATE RestAccount SET name=:name, address=:adr, category=:cat, city=:city, state_post_code=:state WHERE rest_id=:rid");
		$updateQuery->bindParam("name", $sessionData['name']);
		$updateQuery->bindParam("adr", $sessionData['address']);
		$updateQuery->bindParam("cat", $sessionData['category']);
		$updateQuery->bindParam("city", $sessionData['city']);
		$updateQuery->bindParam("state", $sessionData['state_post_code']);
		$updateQuery->bindParam("rid", $restInfo->rest_id);
		$updateQuery->execute();
